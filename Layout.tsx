...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-x-hidden bg-[#f9f7f2]">
      ...
      <header className="sticky top-0 z-[100] bg-[#f9f7f2]/95 backdrop-blur-md border-b border-black/5 px-4 md:px-10 py-3 shadow-sm">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button onClick={()=>setIsMobileMenuOpen(!isMobileMenuOpen)} className="lg:hidden p-2 text-[#0a3d31] hover:bg-black/5 rounded-full transition-colors">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16m-7 6h7"></path></svg>
            </button>
            <div className="flex items-center gap-3 cursor-pointer group" onClick={()=>handleNav('catalog')}>
              <div className="w-10 h-10 md:w-14 md:h-14 rounded-full border-2 border-[#c5a35d] bg-white p-1 shadow-md group-hover:rotate-6 transition-transform">
          ...
          <nav className="hidden lg:flex items-center gap-10 text-[#0a3d31]/50 font-bold uppercase tracking-[0.2em] text-[10px]">
            {navItems.map(item => (
              <button key={item.id} onClick={()=>handleNav(item.id as ViewType)} className={`hover:text-[#c5a35d] relative py-1 transition-all ${currentView === item.id ? 'text-[#0a3d31]' : ''}`}>
                {item.label}
                {currentView === item.id && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-[#c5a35d] animate-fade-in"></span>}
              </button>
            ))}
          </nav>

          <div className="flex items-center gap-2">
            <button onClick={onAdminClick} className="p-2 opacity-30 hover:opacity-100 hover:text-[#c5a35d] transition-all" title="Admin"><ICONS.Admin className="w-5 h-5 md:w-6 md:h-6" /></button>
            <button onClick={() => handleNav('favorites')} className={`p-2 transition-all ${currentView === 'favorites' ? 'text-red-500 scale-110' : 'text-[#0a3d31]/40 hover:text-red-400'}`} title="Favoritos">
            ...
            <button onClick={onLoginClick} className={`p-2 transition-all ${isLoggedIn ? 'text-[#c5a35d] scale-110' : 'text-[#0a3d31]/40 hover:text-[#0a3d31]'}`} title="Usuario"><ICONS.User className="w-5 h-5 md:w-6 md:h-6" /></button>
      ...
      <footer className="bg-[#0a3d31] text-[#f9f7f2] pt-20 pb-10 px-6 border-t-[5px] border-[#c5a35d] mt-12">
        <div className="max-w-7xl mx-auto text-center md:text-left">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-20">
            ...
            <div className="bg-white/5 p-8 rounded-[40px] border border-white/5 text-center md:text-left">
              ...
              <a href={CONTACT_INFO.mapLink} target="_blank" className="text-[9px] font-bold uppercase tracking-widest border-b border-[#c5a35d]/30 pb-1 inline-block hover:text-[#c5a35d] transition-all">Ver Mapa</a>
            </div>
          </div>
